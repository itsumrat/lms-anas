...
import selectors from 'modules/roomSession/roomSessionSelectors';
...
var roomId = 'test';
class LiveAdmin extends Component {
  constructor(props) {
    super(props);
    this.state = {
      currentStream: null,
  ...
  componentWillMount = () => {
    ...
    connection.socketURL =
      'https://digitalschoolapp.herokuapp.com/';
    // 'http://localhost:9001/';
  ...
  ScreenSharingTest = async () => {
    if (
      ...
      if (streamScreenShare) {
        ...
        let streamCamera = await this.getStreamCamera();
  ...
  init = async () => {
    ...
    connection.onstream = async (event) => {
      ...
      if (data.role == Roles.values.teacher) {
        ...
        button1.onclick = this.RoomSlash;
...
export default connect(select)(LiveAdmin);
