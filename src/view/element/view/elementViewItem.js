...
import selectors from "modules/element/elementSelectors";

class ElementViewItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    if (this.props.hasPermissionToRead) {
      return (
        <div key={record.id}>
          <Link to={`/element/${record.id}`}>{record["id"]}</Link>
        </div>
      );
    }
...
export default connect(select)(ElementViewItem);
