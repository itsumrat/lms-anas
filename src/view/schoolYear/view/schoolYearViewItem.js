...
import selectors from "modules/schoolYear/schoolYearSelectors";

class SchoolYearViewItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    if (this.props.hasPermissionToRead) {
      return (
        <div key={record.id}>
          <Link to={`/schoolYear/${record.id}`}>{record["id"]}</Link>
        </div>
      );
    }
...
export default connect(select)(SchoolYearViewItem);
