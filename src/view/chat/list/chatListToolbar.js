...
import destroyActions from "modules/chat/destroy/chatDestroyActions";
import { Link } from "react-router-dom";
import { i18n } from "i18n";
import destroySelectors from "modules/chat/destroy/chatDestroySelectors";

class ChatListToolbar extends Component {
  doExport = () => {
      ...
      <Toolbar>
        ...
        {/* 
                ...
                {this.props.hasPermissionToAuditLogs && (
                    <Link to="/audit-logs?entityNames=Chat">
...
function select(state) {
  return {
    ...
    hasPermissionToEdit: ChatSelectors.selectPermissionToEdit(state),
    hasPermissionToDestroy: ChatSelectors.selectPermissionToDestroy(state),
    hasPermissionToCreate: ChatSelectors.selectPermissionToCreate(state),
    hasPermissionToImport: ChatSelectors.selectPermissionToImport(state),
  };
}

export default connect(select)(ChatListToolbar);
