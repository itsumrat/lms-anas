...
import selectors from "modules/register/registerSelectors";

class RegisterViewItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    if (this.props.hasPermissionToRead) {
      return (
        <div key={record.id}>
          <Link to={`/register/${record.id}`}>{record["id"]}</Link>
        </div>
      );
    }
...
export default connect(select)(RegisterViewItem);
