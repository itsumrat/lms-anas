...
export default class ResponsiblesService {
  static async update(id, data) {
    const response = await graphqlClient.config().mutate({
      mutation: gql`
      mutation MyQuery {
        update_responsible(where: {id: {_eq: ${id}}}, _set: {first_name: "${data.first_name}",last_name: "${data.last_name}"}) {
          affected_rows
          returning {
            id_user
          }
    ...
    let user_id =
      response.data.update_responsible.returning[0].id_user;
  ...
  static async destroyAll(ids) {
    const response = await graphqlClient.config().mutate({
      mutation: gql`
        mutation MyMutation {
          delete_responsible(where: {id: {_eq: ${ids}}}) {
            affected_rows
            returning {
              id_user
            }
    ...
    let id_user =
      response.data.delete_responsible.returning[0].id_user;
  ...
  static async create(data) {
    let userId = await authService.createUser({
      email: data.email,
      phone: data.phone,
      role: role.values.responsible,
    });

    const response = await graphqlClient.config().mutate({
      mutation: gql`
        mutation MyMutation(
          $data: [responsible_insert_input!]!
        ) {
          insert_responsible(objects: $data) {
            affected_rows
            returning {
              id
            }
    ...
    return response.data.insert_responsible;
  }
  ...
  static async import(values, importHash) {
    ...
    return response.data.ResponsiblesImport;
  }
  ...
  static async listAutocomplete(query, limit) {
    ...
    var newMap = response.data.responsible.map((item) => {
      return {
        id: item.id,
