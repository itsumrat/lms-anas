...
function label(name) {
  return i18n(`entities.TimeTableTeacher.fields.${name}`);
}

function enumeratorLabel(name, value) {
  return i18n(`entities.TimeTableTeacher.enumerators.${name}.${value}`);
}

const fields = {
  id: new IdField("id", label("id")),
  matter: new DecimalField("matter", label("matter"), {
    required: true,
  }),
  classroom: new DecimalField("classroom", label("classroom"), {
    required: true,
  }),
