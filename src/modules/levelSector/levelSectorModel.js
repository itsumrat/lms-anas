...
function label(name) {
  return i18n(`entities.LevelSector.fields.${name}`);
}

function enumeratorLabel(name, value) {
  return i18n(
    `entities.LevelSector.enumerators.${name}.${value}`,
  );
}

const fields = {
  id: new IdField('id', label('id')),
  cycle: new DecimalField(
    'cycle',
    label('cycle'),
    {
      required: true,
